...
import sifive.fpgashells.ip.xilinx.artya7mig.{ARTYA7MIGIODDR, ARTYA7MIGIOClocksReset, artya7mig}

case class XilinxArtyA7MIGParams(
  address : Seq[AddressSet]
)

class XilinxArtyA7MIGPads(depth : BigInt) extends ARTYA7MIGIODDR(depth) {
  def this(c : XilinxArtyA7MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxArtyA7MIGIO(depth : BigInt) extends ARTYA7MIGIODDR(depth) with ARTYA7MIGIOClocksReset

class XilinxArtyA7MIGIsland(c : XilinxArtyA7MIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  // require (ranges.size == 1, "DDR range must be contiguous " + ranges.toString())
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxArtyA7MIGIO(depth)
    })
    ...
    val blackbox = Module(new artya7mig(depth))
...
class XilinxArtyA7MIG(c : XilinxArtyA7MIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxArtyA7MIGIsland(c, crossing))
