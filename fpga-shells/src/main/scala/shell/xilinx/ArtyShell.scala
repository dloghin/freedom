...
import sifive.fpgashells.devices.xilinx.xilinxartya7mig._

//-------------------------------------------------------------------------
// ArtyShell
//-------------------------------------------------------------------------

trait HasDDR3 { this: ArtyShell =>

  require(!p.lift(MemoryXilinxDDRKey).isEmpty)
  val ddr = IO(new XilinxArtyA7MIGPads(p(MemoryXilinxDDRKey)))

  def connectMIG(dut: HasMemoryXilinxArtyA7MIGModuleImp): Unit = {
    // Clock & Reset
    dut.xilinxartya7mig.sys_clk_i := sys_clock.asUInt
    mig_clock                    := dut.xilinxartya7mig.ui_clk
    mig_sys_reset                := dut.xilinxartya7mig.ui_clk_sync_rst
    mig_mmcm_locked              := dut.xilinxartya7mig.mmcm_locked
    dut.xilinxartya7mig.aresetn   := mig_resetn
    dut.xilinxartya7mig.sys_rst   := sys_reset

    ddr <> dut.xilinxartya7mig
  }
}
